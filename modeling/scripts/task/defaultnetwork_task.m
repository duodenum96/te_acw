%% Do ACW-TE relationship with default parameters
...
for i = 1:nsim
    ...
    datamatrix(:, :, i) = mean(data, 3); % ERPs

    
    for j = 1:nirois
        for q = 1:nirois
            if j == q
                q_xx(j, q, i) = 1; q_xy(j, q, i) = 1;
                q_yx(j, q, i) = 1; q_yy(j, q, i) = 1;
            else
                x = datamatrix(j, :, i);
                y = datamatrix(q, :, i);
                [q_xx(j, q, i), q_xy(j, q, i), ...
                    q_yx(j, q, i), q_yy(j, q, i)] = greedy(x, y, qmax);
            end
        end
    end
...
save('.gitignore/results/greedy_defaultnetw.mat', 'datamatrix', 'q_xx', 'q_xy', 'q_yx', 'q_yy')
save('.gitignore/results/acws_defaultnetw_task.mat', 'acw0', 'acw50')
