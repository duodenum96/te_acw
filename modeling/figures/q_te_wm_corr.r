...
irois <- roilabels
    ...
    te <- readMat("te_wm.mat")$te # roi x roi x subject
    qs <- readMat("qs_Wrkmem.mat")
    ...
    savename <- paste("figures/","empirical_qxx_te.png", sep = "")
    ggsave(savename)
    ...
    ggplot(finaltibble_selected) + 
        ...
        text = element_text(size = 20), axis.text.x = element_text(angle = 90),
        strip.text = element_text(size=20))
    savename <- paste("figures/", "empirical_qxy_te.png", sep = "")
    ggsave(savename)
