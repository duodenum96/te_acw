...
cd(pipelinedatadir)
addpath(genpath(pipelinedatadir))
...
cfg.lineFreq              = [60 120];
cfg.badsegmode            = 'repnan';
cfg.outputfile            = ['/BICNAS2/ycatal/sourceparcellated/', subjectid, '/', task, '/rmegpreproc/', resultprefix,'_rmegpreproc'];

if ~exist(['/BICNAS2/ycatal/sourceparcellated/', subjectid, '/', task, '/rmegpreproc/'], 'dir')
    mkdir(['/BICNAS2/ycatal/sourceparcellated/', subjectid, '/', task, '/rmegpreproc/'])
end

hcp_repair(cfg);

% hcp_check_pipelineoutput('rmegpreproc', subjectid, scanid, task);
