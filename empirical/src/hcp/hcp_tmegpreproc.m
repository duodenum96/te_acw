...
trialFunName=['trialfun_',task];
%trialFunName=['testtrialfun_',scanmnem];
trialDefFuncFile=['alltrialdefparams_',task];
...
allTrlCfgs{1}.badsegmode = 'repnan';
allTrlCfgs{2}.badsegmode = 'repnan';
...
for iCaseGroup=1:Ncasegroups % AllTrlCfgs
    iCaseGroup
    ...
    if scanid == string('10') || scanid == string('11')
...
for iCaseGroup=1:Ncasegroups
    ...
    cfg  =  [];
    cfg.badchanfile     = [pipelinedatadir, '/', task, '/baddata/', resultprefix,'_',badchansuffix,'.txt'];
    cfg.badsegmfile     = [pipelinedatadir, '/', task, '/baddata/', resultprefix,'_',badsegmsuffix,'.txt'];
    cfg.icainfofile     = [pipelinedatadir, '/', task, '/icaclass/', resultprefix,'_',icainfosuffix];
    ...
    cfg.outputfile      = ['/BICNAS2/ycatal/sourceparcellated/', subjectid, '/', task, '/tmegpreproc/', resultprefix,'_',savesuffix_general,'_',datagroupid];

    if ~exist(['/BICNAS2/ycatal/sourceparcellated/', subjectid, '/', task, '/tmegpreproc/'], 'dir')
        mkdir(['/BICNAS2/ycatal/sourceparcellated/', subjectid, '/', task, '/tmegpreproc/'])
    end
    ...
    cleanTrl=hcp_extract_allfromrun2(cfg);
