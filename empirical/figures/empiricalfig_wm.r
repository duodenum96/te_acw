...
textsize <- 30
tes <- readMat("te_wm.mat")$te # roi x roi x subject

tasksubjs <- readMat("Wrkmem_subjs.mat")$subjs
restsubjs <- readMat("Restin_subjs.mat")$subjs

restacws <- readMat("acw_restmatched_WrkmemTIM_round2.mat")$acw0
restacws2 <- apply(restacws, c(1, 2), mean) # Mean across scans

roilabels <- c("L_V3", "L_46", "L_TPOJ3", "R_V3B", "r_7pM", "R_7PC", "R_LIPv", "R_6d", "R_OP1")
irois <- roilabels
...
savename <- paste("figures/", "empirical_acw_rest.png", sep = "")
ggsave(savename)
...
savename <- paste("figures/", "empirical_te_heatmap.png", sep = "")
ggsave(savename)
...
ggplot(bigt, mapping = aes(x = factor(ROI, level = roif), y = nodedegree, color = ROI)) + 
    ...
    theme(legend.position = "none", axis.text.x = element_text(angle = 90), aspect.ratio=1,
    text = element_text(size = textsize))
savename <- paste("figures/", "empirical_testats.png", sep = "")
ggsave(savename)
...
ggplot(summarytibble) + 
    ...
    text = element_text(size = textsize))
savename <- paste("figures/", "empirical_acw_te.png", sep = "")
ggsave(savename)